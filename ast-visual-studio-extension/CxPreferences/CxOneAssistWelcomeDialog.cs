...
namespace ast_visual_studio_extension.CxPreferences
{
    internal class CxOneAssistWelcomeDialog : Form
    {
        ...
        private bool _disabledAllScannersInThisSession = false;
        private bool _initialCheckboxState = false;

        public bool DisabledAllScannersInThisSession => _disabledAllScannersInThisSession;
        ...
        private void ApplyScannerState()
        {
            ...
            {
                _disabledAllScannersInThisSession = false;
            ...
            {
                _disabledAllScannersInThisSession = true;
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            ...
            // Apply final state so ShowOneAssistWelcomeDialog sees the correct state before persisting.
            // PersistSettings is called by ShowOneAssistWelcomeDialog after ApplyJetBrainsStyleRealtimeScannerPolicy.
            ApplyScannerState();
        }

        private bool IsAtLeastOneScannerEnabled()
        {
            return _settings.UserPrefAscaRealtime ||
                   _settings.UserPrefOssRealtime ||
                   _settings.UserPrefSecretDetectionRealtime ||
                   _settings.UserPrefContainersRealtime ||
                   _settings.UserPrefIacRealtime;
        }
        ...
        private Panel BuildFeatureCard()
        {
            ...
            // Checked state also requires at least one scanner to be enabled in user preferences.
            // Unchecked = disable all scanners for this session without wiping saved UserPref* snapshot.
            _initialCheckboxState = _mcpEnabled && IsAtLeastOneScannerEnabled();
            _enableAllScannersCheckBox = new CheckBox
            {
                AutoSize = true,
                Dock = DockStyle.Top,
                Checked = _initialCheckboxState,
                Enabled = _mcpEnabled,
                BackColor = Color.Transparent,
                ForeColor = _text,
            ...
            if (_mcpEnabled)
            {
                _enableAllScannersCheckBox.CheckedChanged += (_, __) =>
                {
                    ...
                    else if (!_initialCheckboxState)
                    {
                        // Transitioning from unchecked (initial state) to checked means user wants all scanners enabled
                        _welcomeRealtimeCheckboxWasUncheckedOnce = true;
                    }
                };
            }
