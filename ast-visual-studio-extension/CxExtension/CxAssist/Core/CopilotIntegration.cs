...
namespace ast_visual_studio_extension.CxExtension.CxAssist.Core
{
    ...
    internal static class CopilotIntegration
    {
        ...
        {
            // Mode picker button names to search for (flexible matching)
            public static readonly string[] ModePickerNames = { "Chat Mode Picker", "Chat mode", "Mode" };
        ...
        private static void ScheduleAutomatedPromptEntry(string prompt)
        {
            ...
            ScheduleOnIdle(Timing.CopilotOpenDelayMs, () =>
            {
                ...
                // Step 2: Wait for new thread UI to settle, then switch to Agent mode
                ScheduleOnIdle(settleDelay, () =>
                {
                    ...
                    int agentDelay = agentMode ? Timing.NewThreadDelayMs : Timing.PasteDelayMs;

                    // Step 3: Wait for Agent mode to initialize, then paste + submit
        ...
        /// This approach is more reliable because it directly invokes the Agent option
        /// instead of relying on keyboard focus (which often fails with WPF popups).
        ///
        ...
        private static bool TrySwitchToAgentMode()
        {
            try
            {
                ...
                // Step 0: Check if Agent mode is already active (avoid re-switching)
                ...
                // Step 1: Find and click the Mode Picker button
                ...
                Log("UI Automation: Found Mode Picker, invoking it...");

                if (!modePicker.TryGetCurrentPattern(InvokePattern.Pattern, out object pattern))
                {
                    Log("UI Automation: Mode Picker does not support InvokePattern");
                    return false;
                }

                ((InvokePattern)pattern).Invoke();
                System.Threading.Thread.Sleep(700);  // Wait for popup to fully render

                // Step 2: Find and select the Agent option directly (no keyboard navigation)
        ...
        /// <summary>
        /// Finds the Mode Picker button by searching for known names.
        /// </summary>
        ...
        /// <summary>
        /// Checks if Agent mode is already active by examining the Mode Picker button's current display name.
        /// If the button shows "Agent" or contains "Agent" in its name, Agent mode is already active.
        /// </summary>
        private static bool IsAgentModeAlreadyActive(AutomationElement root)
        {
            try
            {
                ...
                bool isAgentActive = modePickerName.ToLower().Contains("agent");

                Log($"UI Automation: Mode Picker current name: '{modePickerName}' (Agent active: {isAgentActive})");
        ...
        /// <summary>
        /// Selects Agent by navigating through dropdown options with arrow keys.
        /// Clears search first, then uses arrow keys to find Agent option.
        /// </summary>
        private static bool SelectAgentDirectly(AutomationElement root)
        {
            try
            {
                // Step 1: Clear any existing search text
                Log("UI Automation: Clearing search box...");
                System.Windows.Forms.SendKeys.SendWait("^a");  // Select all
                System.Threading.Thread.Sleep(100);
                System.Windows.Forms.SendKeys.SendWait("{DELETE}");  // Delete
                System.Threading.Thread.Sleep(500);

                // Step 2: Navigate with arrow keys to find Agent option
                // Try pressing DOWN arrow multiple times and check for Agent after each press
                Log("UI Automation: Navigating dropdown options with arrow keys...");
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    // Check if Agent element is now visible
                    AutomationElement agentElement = FindAgentElement(root);
                    if (agentElement != null)
                    {
                        Log($"UI Automation: Found Agent at navigation step {i + 1}");
                ...
                Log("UI Automation: Agent element not found after navigation");
                Log("UI Automation: Listing available elements for debugging...");
                ListAvailableElements(root);
                return false;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                foreach (AutomationElement el in allElements)
                {
                    try
                    {
                        ...
                        // Look for exact "Agent" (case-insensitive) or mode-like items
                        // Exclude "Search agents", "Search", etc.
                        if (nameLower == "agent" || nameLower.StartsWith("agent "))
                        {
                            string ctType = el.Current.ControlType.ProgrammaticName ?? "";

                            // Prefer MenuItem or ListItem control types (actual mode options)
                ...
                // Second pass: Look for any element with exactly "Agent" name (less restrictive on control type)
                foreach (AutomationElement el in allElements)
                {
                    try
                    {
        ...
        /// <summary>
        /// Lists all elements visible for debugging purposes.
        /// </summary>
            ...
            try
            {
                ...
                foreach (AutomationElement el in allElements)
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(name) && (name.ToLower().Contains("mode") ||
                            name.ToLower().Contains("ask") || name.ToLower().Contains("edit") ||
                            name.ToLower().Contains("debug") || name.ToLower().Contains("agent")))
                        {
