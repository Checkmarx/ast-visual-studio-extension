...
namespace ast_visual_studio_extension.CxExtension.CxAssist.Core.GutterIcons
{
    ...
    internal class CxAssistGlyphTaggerProvider : ITaggerProvider
    {
        // Static instance for external access
        private static CxAssistGlyphTaggerProvider _instance;

        // Cache taggers per buffer to ensure single instance per buffer
        private readonly Dictionary<ITextBuffer, CxAssistGlyphTagger> _taggers =
            new Dictionary<ITextBuffer, CxAssistGlyphTagger>();

        public CxAssistGlyphTaggerProvider()
        {
            System.Diagnostics.Debug.WriteLine("CxAssist: CxAssistGlyphTaggerProvider constructor called - MEF is loading this provider");
            _instance = this;
        }

        public ITagger<T> CreateTagger<T>(ITextBuffer buffer) where T : ITag
        {
            System.Diagnostics.Debug.WriteLine($"CxAssist: CreateTagger called - buffer: {buffer != null}");
            ...
            lock (_taggers)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"CxAssist: CreateTagger - creating NEW tagger for buffer");
                    tagger = new CxAssistGlyphTagger(buffer);
                    _taggers[buffer] = tagger;

                    // Store tagger in buffer properties for external access
                    try
                    {
                        buffer.Properties.AddProperty(typeof(CxAssistGlyphTagger), tagger);
                        System.Diagnostics.Debug.WriteLine($"CxAssist: CreateTagger - tagger stored in buffer properties");
                    }
                    catch
                    {
                        System.Diagnostics.Debug.WriteLine($"CxAssist: CreateTagger - tagger already in buffer properties");
                    }
                    ...
                    buffer.Properties.GetOrCreateSingletonProperty(() => new BufferClosedListener(buffer, () =>
                    {
                        lock (_taggers)
                        {
                            ...
                            buffer.Properties.RemoveProperty(typeof(CxAssistGlyphTagger));
                        }
                    }));
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine($"CxAssist: CreateTagger - returning EXISTING tagger");
                }
        ...
        /// Allows CxAssistPOC or other components to update vulnerabilities
        ...
        /// </summary>
        public static CxAssistGlyphTagger GetTaggerForBuffer(ITextBuffer buffer)
        {
            if (buffer == null)
            {
                System.Diagnostics.Debug.WriteLine("CxAssist: GetTaggerForBuffer - buffer is null");
                return null;
            }
            ...
            if (buffer.Properties.TryGetProperty(typeof(CxAssistGlyphTagger), out CxAssistGlyphTagger tagger))
            {
                System.Diagnostics.Debug.WriteLine("CxAssist: GetTaggerForBuffer - found tagger in buffer properties");
                return tagger;
            }
            ...
            if (_instance != null)
            {
                lock (_instance._taggers)
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine("CxAssist: GetTaggerForBuffer - found tagger in instance cache");
                        return tagger;
                    }
                }
            }
